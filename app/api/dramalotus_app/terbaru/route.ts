...
const SOURCES: SourceConfig[] = [
  { name: "Shortmax", path: "/api/shortmax/latest" },
  { name: "Flickreels", path: "/api/flickreels/trending" },
  { name: "iDrama", path: "/api/idrama/latest" },
  { name: "FlexTV", path: "/api/flextv/latest" },
  { name: "DramaNova", path: "/api/dramanova/latest" },
];
...
function normalizeItem(item: AnyItem, source: SourceConfig, sortOrder: number): AnyItem | null {
...
async function fetchJsonWithTimeout(url: string, timeoutMs = 12_000): Promise<any> {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        "user-agent": "DramaLotus-Backend/1.0",
      },
    });
...
async function fetchLatestItems(req: NextRequest): Promise<AnyItem[]> {
...
async function refreshCache(req: NextRequest): Promise<AnyItem[]> {
  ...
  refreshing = fetchLatestItems(req)
    .then((items) => {
      if (items.length > 0) {
        cache = {
          savedAt: Date.now(),
...
export async function GET(req: NextRequest) {
  ...
  // Kalau ada cache lama, kembalikan langsung supaya Flutter tidak loading lama,
  // lalu refresh diam-diam di background.
