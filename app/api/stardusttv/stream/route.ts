...
import { extractEpisodeStreamUrl, fetchStardustJson } from "../_shared";
...
function buildChildProxyUrl(
  ...
  return `/api/stardusttv/stream?token=${encodeURIComponent(token)}`;
}
...
async function resolveStardustStreamUrl(
  videoId: string,
  episode: string,
): Promise<string> {
  if (!videoId.trim() || !episode.trim()) return "";
  ...
  return extractEpisodeStreamUrl(payload);
}
...
async function handleStream(request: NextRequest) {
  ...
  if (!videoId || !episode) {
    return NextResponse.json(
      { error: "Missing StardustTV videoId/episode." },
      { status: 400, headers: buildCorsHeaders("application/json") },
    );
  }

  const resolvedUrl = await resolveStardustStreamUrl(videoId, episode);

  if (!resolvedUrl) {
    return NextResponse.json(
      { error: "Failed to resolve StardustTV stream." },
  ...
  return new NextResponse(null, {
    status: 307,
    headers: {
      Location: `/api/stardusttv/stream?token=${encodeURIComponent(
        initialToken,
      )}`,
      ...buildCorsHeaders(),
    },
...
export async function GET(request: NextRequest) {
  const accessError = await requireStardustAccess(request);
