...
async function resolveMicrodramaStreamUrl(
  dramaId: string,
  episode: string,
): Promise<string> {
...
async function handleStream(request: NextRequest) {
  ...
  if (!dramaId || !episode) {
    return NextResponse.json(
      { error: "Missing MicroDrama dramaId/episode." },
      { status: 400, headers: buildCorsHeaders("application/json") },
    );
  }

  const resolvedUrl = await resolveMicrodramaStreamUrl(dramaId, episode);

  if (!resolvedUrl) {
    return NextResponse.json(
      { error: "Failed to resolve MicroDrama stream." },
  ...
  return new NextResponse(null, {
    status: 307,
    headers: {
      Location: `/api/microdrama/stream?token=${encodeURIComponent(
        initialToken,
      )}`,
      ...buildCorsHeaders(),
    },
...
export async function GET(request: NextRequest) {
  const accessError = await requireMicrodramaAccess(request);
