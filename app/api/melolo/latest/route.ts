...
const PRIMARY_URL = "https://api.sansekai.my.id/api/melolo/latest";
  ...
  try {
    ...
    if (!fallback.ok) {
      return NextResponse.json(
        {
          error: `Melolo latest failed. primary=${primary.status} fallback=${fallback.status}`,
        },
    ...
    console.error("Melolo latest route error:", error);
