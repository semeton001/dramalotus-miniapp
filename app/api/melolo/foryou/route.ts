...
const PRIMARY_URL = "https://api.sansekai.my.id/api/melolo/foryou?offset=1";
  ...
  try {
    ...
    if (!fallback.ok) {
      return NextResponse.json(
        {
          error: `Melolo foryou failed. primary=${primary.status} fallback=${fallback.status}`,
        },
    ...
    console.error("Melolo foryou route error:", error);

    return NextResponse.json(
      { error: "Failed to load Melolo ForYou feed" },
      { status: 500 },
    );
  }
}