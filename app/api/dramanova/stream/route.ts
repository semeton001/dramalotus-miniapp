...
import { resolveDramaNovaVideoUrl } from "../_shared";
...
async function requireDramaNovaAccess(request: NextRequest) {
...
async function handleStream(request: NextRequest) {
  ...
  if (!fileId) {
    return NextResponse.json(
      { ok: false, source: "dramanova", error: "Missing fileId" },
      { status: 400, headers: buildCorsHeaders("application/json") },
    );
  }

  const resolvedUrl = await resolveDramaNovaVideoUrl(fileId);
    ...
    return NextResponse.json(
      {
        ...
        source: "dramanova",
        fileId,
        error: "No playable DramaNova stream resolved",
      },
  ...
  const initialPayload: VerifiedStreamToken = {
    ...
    episodeKey: `${fileId}:${episodeNumber}`,
    url: resolvedUrl,
  ...
  return new NextResponse(null, {
    status: 307,
    headers: {
      Location: `/api/dramanova/stream?token=${encodeURIComponent(initialToken)}`,
      ...buildCorsHeaders(),
    },
  });
}
...
export async function GET(request: NextRequest) {
  const accessError = await requireDramaNovaAccess(request);
