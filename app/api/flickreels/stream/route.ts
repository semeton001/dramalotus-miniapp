...
const ALLOWED_MEDIA_EXTENSIONS = [".m3u8", ".ts", ".mp4", ".m4s", ".key", ".aac"];
...
function shouldRewritePlaylistLine(line: string): boolean {
...
function rewritePlaylist(
  ...
  const proxyBase = new URL("/api/flickreels/stream", request.url);

  return body
    .split(/\r?\n/)
    .map((line) => {
      if (!shouldRewritePlaylistLine(line)) {
        return line;
      }
