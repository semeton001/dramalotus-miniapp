...
import { resolveBiliTVVideoUrl } from "../_shared";
...
function buildUpstreamHeaders(request: NextRequest, upstreamUrl: URL): Headers {
  ...
  headers.set("Accept", "*/*");
  headers.set("Origin", "https://www.bilitv.com");
  headers.set("Referer", "https://www.bilitv.com/");
  headers.set(
    "User-Agent",
...
function absolutizeHlsLine(line: string, baseUrl: string): string {
...
function proxifyHlsLine(
  ...
  const absolute = absolutizeHlsLine(cleanedLine, baseUrl);
  const trimmed = absolute.trim();

  if (!trimmed || trimmed.startsWith("#")) return absolute;

  const segmentToken = createStreamToken({
    provider: "bilitv",
  ...
  return `/api/bilitv/stream?token=${encodeURIComponent(segmentToken)}`;
}

function isHlsContent(rawUrl: string, contentType: string): boolean {
  return (
...
async function requireBiliTVAccess(request: NextRequest) {
...
async function proxyMedia(
  ...
  if (request.method === "HEAD") {
    return new NextResponse(null, {
      ...
      headers: copyUpstreamHeaders(
        upstream,
        contentType || (rawUrl.includes(".m3u8") ? "application/x-mpegurl" : null),
      ),
    });
  }

  if (isHlsContent(rawUrl, contentType)) {
    const text = await upstream.text();
    const playlist = text
      .split(/\r?\n/)
      .map((line) => proxifyHlsLine(line, rawUrl, payload))
    ...
    return new NextResponse(playlist, {
      status: 200,
      headers: {
        ...buildCorsHeaders("application/x-mpegurl"),
  ...
  return new NextResponse(upstream.body, {
    status: upstream.status,
    headers: copyUpstreamHeaders(upstream, contentType || "video/mp2t"),
  });
}
...
async function handleStream(request: NextRequest) {
  ...
  const episodeNumber =
    request.nextUrl.searchParams.get("episodeNumber")?.trim() || episode || "1";

  if (!dramaId || !episode) {
    return NextResponse.json(
      { ok: false, source: "bilitv", error: "Missing dramaId or episode" },
      { status: 400, headers: buildCorsHeaders("application/json") },
    );
  }

  const resolvedUrl = await resolveBiliTVVideoUrl(dramaId, episode);
    ...
    return NextResponse.json(
      {
        ...
        dramaId,
        episode,
        error: "No playable BiliTV stream resolved",
      },
  ...
  return new NextResponse(null, {
    status: 307,
    headers: {
      Location: `/api/bilitv/stream?token=${encodeURIComponent(initialToken)}`,
      ...buildCorsHeaders(),
    },
  });
}
...
export async function GET(request: NextRequest) {
  const accessError = await requireBiliTVAccess(request);
