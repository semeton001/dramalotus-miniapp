...
async function resolveFundramaStreamUrl(
  dramaId: string,
  episode: string,
): Promise<string> {
  ...
  return pickBestVideoUrl(rawEpisode.ptitl);
}

async function requireFundramaAccess(request: NextRequest) {
...
async function handleStream(request: NextRequest) {
  ...
  if (!dramaId || !episode) {
    return NextResponse.json(
      { error: "Missing FunDrama dramaId/episode." },
      { status: 400, headers: buildCorsHeaders("application/json") },
    );
  }

  const resolvedUrl = await resolveFundramaStreamUrl(dramaId, episode);

  if (!resolvedUrl) {
    return NextResponse.json(
      { error: "Failed to resolve FunDrama stream." },
  ...
  return new NextResponse(null, {
    status: 307,
    headers: {
      Location: `/api/fundrama/stream?token=${encodeURIComponent(initialToken)}`,
      ...buildCorsHeaders(),
    },
  });
}
...
export async function GET(request: NextRequest) {
  const accessError = await requireFundramaAccess(request);
